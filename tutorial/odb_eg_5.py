...
log.info("query: Find Colin's Friends who are also Customers, and the Countries they are from")
query_str = "MATCH {Class: Profiles, as: profile, where: (Name='Colin' AND Surname='OrientDB')}" \
            "-HasFriend-{Class: Profiles, as: friend}" \
            "<-HasProfile-{class: Customers, as: customer}" \
            "-IsFromCountry->{Class: Countries, as: country} " \
            "RETURN $pathelements"
...
query_str = "MATCH {Class: Profiles, as: profile, where: (Name='Colin' AND Surname='OrientDB')}" \
            ...
            "friend.Surname as Friend_Surname, customer.@Rid as Customer_RID, " \
            "customer.OrderedId as Customer_OrederedId, country.Name as FriendIsFrom"
